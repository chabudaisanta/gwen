...
struct FenwickTree {
     ...
     */
    template <class F>
    i32 max_right(F&& f) const { return max_right_internal(Abel::e(), std::forward<F>(f)); }
     ...
     * @return i32 条件を満たす最大の r (l <= r <= N)。※ただし条件によっては l 未満の値も返しうる
     */
    template <class F>
    i32 max_right(i32 l, F&& f) const { return max_right_internal(Abel::inv(sum(l)), std::forward<F>(f)); }

private:
    template <class F>
    i32 max_right_internal(S initial, F&& f) const {
        S s = initial;
        i32 x = 0;
