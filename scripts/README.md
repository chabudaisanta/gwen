...
## 4. `check_docs.py`
公開ドキュメントの目次・仕様書・ヘッダーファイルの対応関係を検査するスクリプトです。ドキュメントまたは公開ヘッダーを追加・移動・削除したときに実行してください。
...
### 検査内容
1. `.verify-helper/docs/index.md` のリンク先が存在し、公開ヘッダーが重複なく掲載されていることを確認します。
2. `include/gwen/` 配下のすべての公開ヘッダーに、対応する `doc/` 配下の仕様書があることを確認します。
3. 各仕様書の `documentation_of` と配置パスが、対応するヘッダーと一致することを確認します。
