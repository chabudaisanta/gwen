...
- **自作ヘッダのインクルード**: `gwen` 内部の自作ヘッダファイルをインクルードする際は、必ず `#include <gwen/...>` のようにアングルブラケット形式を使用します。これは `competitive-verifier` が Markdown の使用例を検証する際、相対パス (`"..."`) だとパス解決に失敗してエラーになることを防ぐためです。
