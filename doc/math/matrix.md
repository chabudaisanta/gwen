...
`field` コンセプトを満たす任意の体 (Field) を要素に持つことができます（例えば、有理数、実数、あるいは `DynamicModInt64` などのモジュラ整数など）。
...
掃き出し法（ガウスの消去法）を用いて行列式を求めます。要素型 `T` は `field` を満たす必要があります。

**制約**

- 正方行列であること。
- 要素型 `T` は `field` を満たすこと。
