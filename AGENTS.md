...
- グラフや代数構造などの汎用コンポーネントは C++23 `concept` を用いて厳密な型制約を設けてください（※ただし「コンセプト設計原則」を遵守すること）。
- ビルド成果物は `.build/` へ出力し、AIエージェントが一時的なファイルを作成する場合は `temp/` ディレクトリを使用してください。
</RULE>

<RULE>
## コンセプト設計原則 (Concept Design Principles)
- **概念を表すコンセプトを定義すること**: コンセプトは代数系（`monoid`, `abel`, `semiring`, `ring`, `field` 等）やグラフ理論（`graph`, `edge` 等）のような、計算機科学・数学において自立した本質的な「概念」を表す場合にのみ定義してください。
- **特定関数・構造体の適用可否チェッカーは作成禁止**: 単にある特定の関数や構造体に渡せる（適用できる）という意味合いだけのアドホックなコンセプト（例: `xxx_value`, `xxx_graph`, `xxx_weight`, `xxx_action`, `xxx_aggregator` 等）は作成しないでください。特定の関数の引数は、本質的な概念（代数構造等）や C++ 標準コンセプト（`<concepts>`）を用いて受けるか、通常のテンプレート引数として受けてください。
</RULE>
