...
public:
    ...
    is_type& operator>>(i32& dest) { dest = get_i32(); return *this; }
    is_type& operator>>(u32& dest) { dest = get_u32(); return *this; }
    is_type& operator>>(i64& dest) { dest = get_i64(); return *this; }
    is_type& operator>>(u64& dest) { dest = get_u64(); return *this; }
    is_type& operator>>(i128& dest) { dest = get_i128(); return *this; }
    is_type& operator>>(u128& dest) { dest = get_u128(); return *this; }
    is_type& operator>>(char& dest) { dest = get_char(); return *this; }
    is_type& operator>>(std::string& dest) { dest = get_token(); return *this; }

    template<std::signed_integral INT>
    is_type& operator>>(INT& dest) { dest = static_cast<INT>(get_i64()); return *this; }
    template<std::unsigned_integral UINT>
    is_type& operator>>(UINT& dest) { dest = static_cast<UINT>(get_u64()); return *this; }
...
public:
    ...
    void push_u32(u32 x) {
        ...
        else if(x >= 1) push_token(TB.NLZ[x]);
        else push_1byte('0');
    }

    void push_i32(i32 x) {
        if(0 <= x) push_u32(static_cast<u32>(x));
        else {
            push_1byte('-');
    ...
    os_type& operator<<(i32 tg) { push_i32(tg); return *this; }
    os_type& operator<<(u32 tg) { push_u32(tg); return *this; }
    os_type& operator<<(i64 tg) { push_i64(tg); return *this; }
    os_type& operator<<(u64 tg) { push_u64(tg); return *this; }
    // TODO: i128, u128
    os_type& operator<<(const std::string& tg) { push_token(tg.c_str()); return *this; }
    os_type& operator<<(const char* tg) { push_token(tg); return *this; }
    os_type& operator<<(char tg) { push_1byte(tg); return *this; }

    template<std::signed_integral INT>
    os_type& operator<<(INT tg) { push_i64(tg); return *this; }
    template<std::unsigned_integral UINT>
    os_type& operator<<(UINT tg) { push_u64(tg); return *this; }
