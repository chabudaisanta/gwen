...
template <i32 Rank>
using mdspan_extents = std::dextents<i32, Rank>;

template <typename T, i32 Rank>
using mdspan_type = std::mdspan<T, mdspan_extents<Rank>>;

template <i32 Rank>
constexpr mdspan_extents<Rank> make_extents(const std::array<i32, Rank>& shape) {
    return [&]<size_t... Is>(std::index_sequence<Is...>) { return mdspan_extents<Rank>{shape[Is]...}; }(
        std::make_index_sequence<Rank>{});
}

#else

template <typename IndexType, i32 Rank>
struct mdspan_extents_impl {
    ...
    constexpr size_t offset(Is... indices) const {
...
struct mdspan_impl {
    ...
    template <typename... Is>
    constexpr T& operator()(Is... indices) const {
        return ptr[ext.offset(indices...)];
    }
...
template <i32 Rank>
using mdspan_extents = mdspan_extents_impl<i32, Rank>;

template <typename T, i32 Rank>
using mdspan_type = mdspan_impl<T, mdspan_extents<Rank>>;
